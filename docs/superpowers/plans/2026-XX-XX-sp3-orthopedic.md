# SP-3: Orthopedic Module Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Implement the interactive skeleton — body regions + bones model, skeleton API, bone events CRUD, and the SVG-based interactive skeleton frontend.

**Architecture:** Three layers — Python data model in FastAPI, REST endpoints under `/api/patients/{id}/skeleton`, vanilla JS + SVG skeleton frontend.

**Tech Stack:** Python/SQLAlchemy, Pydantic, FastAPI, vanilla JS, SVG, CSS

**Spec:** `docs/superpowers/specs/2026-XX-XX-clinical-platform-design.md` (especially §6.3, §7.4, §10.2 Skeleton)

**Depends on:** SP-1 (schema, auth context, Express proxy). Can run in parallel with SP-2.

## Global Constraints

- A patient's skeleton is auto-created the first time it's accessed (no separate POST endpoint needed)
- Standard body regions: head, neck, spine (cervical/thoracic/lumbar/sacrum), shoulders (L/R), arms-upper (L/R), arms-lower (L/R), hands (L/R), ribs, pelvis, hips (L/R), legs-upper (L/R), legs-lower (L/R), feet (L/R)
- Bone events are immutable once created (soft delete via status='archived')
- Express proxy adds `x-user-id` / `x-user-role` / `x-clinic-id` headers — FastAPI reads them via `get_user_context()`

---

## Task 1: Database Migration — Orthopedic Tables

**Files:**
- Create: `clinical-platform/database/migrations/003_orthopedic.sql`
...
```bash
docker compose exec -T postgres psql -U clinical -d clinical_platform < database/migrations/003_orthopedic.sql
```
...
git commit -m "feat(db): orthopedic chart, body_regions, bones, bone_events tables"
```

---

## Task 2: Skeleton API — Read & Auto-seed

**Files:**
- Create: `clinical-platform/backend/fastapi/app/api/orthopedic.py`
- Modify: `clinical-platform/backend/fastapi/app/main.py` (include the new router)

- [ ] **Step 1: Write orthopedic.py**
...
- [ ] **Step 2: Modify main.py to include the orthopedic router**

Append inside `app/main.py`:

```python
from .api import orthopedic
...
```bash
git add backend/fastapi/app/api/orthopedic.py backend/fastapi/app/main.py
git commit -m "feat(orthopedic): skeleton API with auto-seed and bone event CRUD"
```

---

## Task 3: Interactive Skeleton Frontend

**Files:**
- Create: `clinical-platform/frontend/js/pages/skeleton.js`
- Create: `clinical-platform/frontend/css/skeleton.css`
- Create: `clinical-platform/frontend/js/components/skeleton-svg.js`

- [ ] **Step 1: Create skeleton.css**
...
.skeleton-svg-container .bone-region {
  cursor: pointer;
  transition: all 0.2s;
}

.skeleton-svg-container .bone-region:hover {
  stroke: var(--color-primary);
  stroke-width: 2;
}

.skeleton-state-normal { fill: #ffffff; stroke: #0f172a; stroke-width: 1; }
.skeleton-state-fracture { fill: #fca5a5; stroke: #991b1b; stroke-width: 2; }
.skeleton-state-under_treatment { fill: #fed7aa; stroke: #c2410c; stroke-width: 1.5; }
.skeleton-state-follow_up { fill: #fde68a; stroke: #b45309; stroke-width: 1.5; }
.skeleton-state-healing { fill: #bbf7d0; stroke: #15803d; stroke-width: 1.5; }
.skeleton-state-surgical { fill: #c4b5fd; stroke: #5b21b6; stroke-width: 1.5; }
.skeleton-state-chronic { fill: #e9d5ff; stroke: #6b21a8; stroke-width: 1.5; }
.skeleton-state-treated { fill: #e0f2fe; stroke: #075985; stroke-width: 1; }
...
.bone-detail-panel.open { transform: translateX(0); }
```

- [ ] **Step 2: Create skeleton-svg.js**
...
- [ ] **Step 3: Create skeleton.js (page with bone detail panel)**
...
```bash
git add frontend/
git commit -m "feat(orthopedic): interactive skeleton SVG and event panel"
```
...
|------|------------|--------|
| 1 | Orthopedic tables migration | |
| 2 | Skeleton API (read, auto-seed, event CRUD) | |
| 3 | Interactive skeleton frontend | |
